...
export function createQueueResources(db: ReturnType<typeof drizzle>) {
  return {
    ...
    get: async (id: string) => {
      const [res] = await db.select().from(queues).where(eq(queues.id, id))
      return res || null
    },
    listByUser: async (userId: string) => {
      return db.select().from(queues).where(eq(queues.userId, userId))
    },
